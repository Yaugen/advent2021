...
const ops = {
	0: (...args) => args.reduce((acc, item) => acc + item, 0),
	1: (...args) => args.reduce((acc, item) => acc * item, 1),
	2: Math.min,
	3: Math.max,
	5: (a, b) => (a > b ? 1 : 0),
	6: (a, b) => (a < b ? 1 : 0),
...
const resolve = (p) => {
	if (p.type === 4) {
		...
		return ops[p.type](...p.subPackets.map(resolve));
	}
};

console.log(resolve(parsedPacket));
